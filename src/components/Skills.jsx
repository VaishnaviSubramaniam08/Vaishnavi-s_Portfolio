...
const Skills = () => {
  return (
    <section id="skills" className="py-32 relative bg-slate-50 dark:bg-[#0B1120] transition-colors duration-300 overflow-hidden min-h-[80vh] flex flex-col justify-center">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        ...
        <div className="text-center mb-24">
          <motion.h2
            ...
            viewport={{ once: true }}
            className="text-4xl md:text-5xl font-bold font-display text-slate-900 dark:text-white mb-4 tracking-tight"
          >
            Technical Arsenal
          </motion.h2>
          <motion.p
        ...
        <div className="relative max-w-4xl mx-auto flex flex-wrap justify-center items-center content-center gap-x-6 gap-y-10 md:gap-x-10 md:gap-y-14 p-10 h-auto min-h-[400px] rounded-3xl border border-gray-200 dark:border-slate-700/50 bg-white/50 dark:bg-slate-800/30 backdrop-blur-3xl shadow-lg dark:shadow-[0_0_40px_rgba(0,0,0,0.3)]">
          {FLOATING_SKILLS.map((skill, index) => {
            ...
            return (
              <motion.div
                key={skill.name}
                className="relative flex items-center gap-3 px-6 py-3 rounded-full border border-gray-300 dark:border-white/20 bg-white dark:bg-white/5 backdrop-blur-md text-slate-800 dark:text-white font-sans font-bold tracking-widest text-sm cursor-pointer whitespace-nowrap shadow-[0_0_10px_rgba(0,0,0,0.05)] dark:shadow-[0_0_15px_rgba(255,255,255,0.02)] transition-colors"
                initial={{
                  opacity: 0,
                  scale: 0.8,
                  rotate: skill.rotate,
              ...
              >
                <img 
                  ...
                  alt={`${skill.name} icon`} 
                  className={`w-6 h-6 object-contain ${skill.invertOnDark ? 'dark:invert' : ''}`}
                />
